...
LEFT OUTER JOIN ( -- HOLDOUT PERIOD CALCULATIONS
  ...
  WHERE 
    ...
    TO_DATE(p.invoicedate) >= DATE_SUB(q.current_dt, {holdout_days}) AND  -- LIMIT THIS QUERY TO DATA IN THE HOLDOUT PERIOD
...
import mlflow 
# set personal mlflow experiment to log hyperopt runs
useremail = dbutils.notebook.entry_point.getDbutils().notebook().getContext().userName().get()
experiment_name = f"/Users/{useremail}/CLV_experiment"
...
# MAGIC 
# MAGIC To demonstrate this, we must first train a lifetime model.  We'll use a ParetoNBD introduced in the prior notebook for this:
...
conda_env['dependencies'][2]['pip'] += [f'lifetimes=={lifetimes.__version__}'] 
